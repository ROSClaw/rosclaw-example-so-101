...
enum GatewayRosclawPromptOracle {
    ...
    nonisolated static var cartesianCommandGuidance: String {
        ...
        1. Prefer `/follower/move_to_cartesian_goal` or any service whose final path component is `move_to_cartesian_goal`, type `so101_pose_registry_interfaces/srv/MoveToCartesianGoal`.
        2. If that service is available, call it with `{pose: <requested PoseStamped>, duration_sec: 3.0}` and use the returned `result.reason`, `result.accepted`, and `result.resolved_pose`.
        3. Otherwise prefer `/follower/cartesian_goal` or any topic whose final path component is `cartesian_goal`, type `geometry_msgs/msg/PoseStamped`.
        4. Publish the requested pose to the discovered topic.
        5. Use `follower/base_link` as the frame id when the SO-101 follower endpoint is present; otherwise preserve the same `base_link` suffix if the discovered endpoint is namespaced.
        6. If the publish or service call succeeds but state refresh fails, still return `ok:true` with `state:null` and mention that state is unavailable in `summary`.
        """
    }
...
actor GatewayRobotExecutor {
    ...
    private func prompt(
        ...
        case .moveToCartesian(let x, let y, let z):
            ...
            - headsetNodeId: \(self.nodeID)
            - requestedPositionMeters: {"x": \(Self.format(x)), "y": \(Self.format(y)), "z": \(Self.format(z))}
            - workspaceSnapshot:
            \(workspaceSnippet)
            ...
            7. Use this fixed end-effector orientation:
               {"x": 0.0, "y": 0.707107, "z": 0.0, "w": 0.707107}
            8. Set the requested position exactly to the requested coordinates.
            9. If the move service returns a resolved pose, report in `summary` that the robot is moving to the resolved safe pose, not the raw request.

            After the publish attempt, refresh robot state using these rules:
            \(GatewayRosclawPromptOracle.stateResolutionGuidance)

            Return ONLY valid JSON:
            {"schemaVersion":"1","kind":"command_result","intentId":"\(intentID)","ok":true,"summary":"cartesian goal published","errorCode":null,"estopLatched":false,"state":{"joint_order":[],"joint_positions":{},"gripper_position":null,"gripper_open_fraction":null,"tool_pose":{"frame_id":"follower/base_link","position":{"x":\(Self.format(x)),"y":\(Self.format(y)),"z":\(Self.format(z))},"orientation":{"x":0.0,"y":0.707107,"z":0.0,"w":0.707107}}}}

            If the command succeeds but state cannot be refreshed, return:
            {"schemaVersion":"1","kind":"command_result","intentId":"\(intentID)","ok":true,"summary":"cartesian goal published; state unavailable","errorCode":null,"estopLatched":false,"state":null}

            If publishing fails, return:
